...
export function Analytics() {
  ...
  const [stats, setStats] = useState({
    ...
    classesThisWeek: 0,
    avgClassDuration: 0,
    top3Movements: ['No data yet'] as string[],
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      setStats({
        ...
        top3Movements: summaryResponse.data.top_3_movements || ['No data yet'],
        coveragePercentage: summaryResponse.data.coverage_percentage || 0,
      });
...
Avg Class Duration (min),${stats.avgClassDuration}`;
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {/* Early Skip Analytics - Admin Only Section (December 29, 2025) */}
      {user?.is_admin && (
        <AdminEarlySkipAnalytics userId={user.id} />
      )}
    </div>
  );
}
...
// ==============================================================================
// ADMIN EARLY SKIP ANALYTICS COMPONENT - December 29, 2025
// ==============================================================================

interface AdminEarlySkipAnalyticsProps {
  userId: string;
}
...
function AdminEarlySkipAnalytics({ userId }: AdminEarlySkipAnalyticsProps) {
  ...
  if (isLoading) {
    return (
      <Card className="mt-8">
        ...
        <CardBody className="p-6">
          <div className="flex items-center justify-center h-32">
            <div className="text-cream/60">Loading early skip analytics...</div>
          </div>
        </CardBody>
      </Card>
    );
  ...
  return (
    <div className="mt-8 space-y-6">
      ...
      <div className="bg-burgundy/20 border-2 border-burgundy p-4 rounded-lg">
        <div className="flex items-center gap-3">
          <span className="bg-burgundy px-3 py-1 rounded text-sm font-bold">ADMIN ONLY</span>
          <h2 className="text-2xl font-bold text-cream">Early Skip Analytics</h2>
        </div>
        <p className="text-cream/70 mt-2">
          Track which sections users skip before the minimum engagement threshold (60s for sections, 20s for transitions)
        </p>
      </div>
      ...
      <Card>
        ...
        <CardBody className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-burgundy/10 p-4 rounded-lg">
              <p className="text-cream/60 text-xs font-medium mb-1">Total Section Plays</p>
              <p className="text-3xl font-bold text-cream">{analytics.overall_stats.total_plays}</p>
            </div>
            <div className="bg-burgundy/10 p-4 rounded-lg">
              <p className="text-cream/60 text-xs font-medium mb-1">Total Early Skips</p>
              <p className="text-3xl font-bold text-cream">{analytics.overall_stats.total_early_skips}</p>
            </div>
            <div className="bg-burgundy/10 p-4 rounded-lg border-2 border-burgundy/50">
              <p className="text-cream/60 text-xs font-medium mb-1">Overall Skip Rate</p>
              <p className="text-3xl font-bold text-cream">{analytics.overall_stats.overall_skip_rate_pct}%</p>
            </div>
          </div>
        </CardBody>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Skip Rates by Section Type</CardTitle>
        </CardHeader>
        <CardBody className="p-6">
          {analytics.by_section_type.length === 0 ? (
            <div className="text-center text-cream/60 py-8">
              No section play data yet. Users need to start classes for data to appear.
            </div>
          ) : (
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-cream/30">
                    <th className="text-left py-3 px-4 text-cream font-semibold">Section Type</th>
                    <th className="text-center py-3 px-4 text-cream font-semibold">Total Plays</th>
                    <th className="text-center py-3 px-4 text-cream font-semibold">Early Skips</th>
                    <th className="text-center py-3 px-4 text-cream font-semibold">Skip Rate %</th>
                    <th className="text-center py-3 px-4 text-cream font-semibold">Avg Duration</th>
                    <th className="text-center py-3 px-4 text-cream font-semibold">Planned Duration</th>
                  </tr>
                </thead>
                <tbody>
                  {analytics.by_section_type.map((section) => (
                    <tr key={section.section_type} className="border-b border-cream/10 hover:bg-burgundy/10">
                      <td className="py-3 px-4 text-cream capitalize">{section.section_type}</td>
                      <td className="py-3 px-4 text-center text-cream/80">{section.total_plays}</td>
                      <td className="py-3 px-4 text-center text-cream/80">{section.early_skips}</td>
                      ...
                      <td className="py-3 px-4 text-center text-cream/80">{section.avg_duration_seconds}s</td>
                      <td className="py-3 px-4 text-center text-cream/80">{section.avg_planned_duration}s</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Top 10 Most-Skipped Movements</CardTitle>
        </CardHeader>
        <CardBody className="p-6">
          {analytics.by_movement.length === 0 ? (
            <div className="text-center text-cream/60 py-8">
              No movement data yet (requires minimum 10 plays per movement for statistical significance)
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-cream/30">
                    <th className="text-left py-3 px-4 text-cream font-semibold">Rank</th>
                    <th className="text-left py-3 px-4 text-cream font-semibold">Movement Name</th>
                    <th className="text-center py-3 px-4 text-cream font-semibold">Total Plays</th>
                    <th className="text-center py-3 px-4 text-cream font-semibold">Early Skips</th>
                    <th className="text-center py-3 px-4 text-cream font-semibold">Skip Rate %</th>
                    <th className="text-center py-3 px-4 text-cream font-semibold">Avg Duration</th>
                ...
                <tbody>
                  {analytics.by_movement.map((movement, index) => (
                    <tr key={movement.movement_id} className="border-b border-cream/10 hover:bg-burgundy/10">
                      <td className="py-3 px-4 text-cream font-bold">#{index + 1}</td>
                      <td className="py-3 px-4 text-cream">{movement.movement_name}</td>
                      <td className="py-3 px-4 text-center text-cream/80">{movement.total_plays}</td>
                      <td className="py-3 px-4 text-center text-cream/80">{movement.early_skips}</td>
                      <td className="py-3 px-4 text-center">
                        <span
                          className={`px-2 py-1 rounded text-sm font-semibold ${
                      ...
                      <td className="py-3 px-4 text-center text-cream/80">{movement.avg_duration_seconds}s</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {/* SQL Transparency - Data Sources */}
      <Card>
        <CardHeader>
          <CardTitle>Data Source Transparency - SQL Queries</CardTitle>
        </CardHeader>
        <CardBody className="p-6">
          <p className="text-cream/70 mb-4 text-sm">
            All analytics above are derived from the database tables below. Copy any SQL query to run it directly in Supabase SQL Editor for validation.
          </p>
          <div className="space-y-4">
            {analytics.data_sources.map((source, index) => (
              <div key={index} className="bg-burgundy/10 border border-cream/20 rounded-lg p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <p className="text-cream font-semibold text-sm">Query #{index + 1}</p>
                    <p className="text-cream/60 text-xs mt-1">{source.description}</p>
                    <p className="text-cream/40 text-xs mt-1">
                      <span className="font-mono">Table: {source.table_name}</span>
                    </p>
                  </div>
                  <button
                    onClick={() => copySQL(source.sql_query, source.description)}
                    className="px-3 py-1.5 bg-burgundy text-cream rounded text-xs hover:bg-burgundy-dark transition-colors font-semibold flex items-center gap-2"
                  >
                    {copiedQuery === source.description ? (
                      <>
                ...
                <div className="bg-burgundy-dark p-3 rounded border border-cream/10 mt-2 max-h-64 overflow-y-auto">
