...
export function MovementDisplay({ item, isPaused = false }: MovementDisplayProps) {
  ...
  useEffect(() => {
    ...
    // Calculate scroll speed based on duration (54% slower for readability - user requested 50% + 7% - 4%)
    const duration = item.duration_seconds * 1000 * 1.541; // Convert to ms, 54% slower (1.541x duration)
